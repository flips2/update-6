  ...
  const calculateUserAnalytics = (sessions: TradingSession[], trades: Trade[]): UserAnalytics => {
    ...
    trades.forEach(trade => {
      if (trade.sl && trade.open_price && trade.profit_loss !== undefined) {
        // Calculate the initial risk (distance from entry to stop loss)
        const riskAmount = Math.abs(trade.open_price - trade.sl);
        
        // Only calculate R-multiple if we have a valid risk amount
        if (riskAmount > 0) {
          const rMultiple = trade.profit_loss / riskAmount;
